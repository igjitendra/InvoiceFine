...
export function CatalogRow({ item, onPress }: { item: CatalogItem; onPress: () => void }) {
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={`${item.name}, ${formatPaise(item.sellingPricePaise)}`} onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>

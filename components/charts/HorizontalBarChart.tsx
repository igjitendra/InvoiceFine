...
export function HorizontalBarChart({
  ...
  return (
    ...
    >
      {data.map((item, i) => {
        const y = i * row,
          w = Math.max(4, (item.value / max) * 190);
        return (
          <GRow
            key={item.id}
            item={item}
            ...
            width={w}
            palette={palette}
            color={item.color ?? palette.colors[i % palette.colors.length]}
            onSelect={onSelect}
          />
        );
      })}
...
function GRow({
  ...
  return (
    <>
      <SvgText x="0" y={y + 14} fill={palette.text} fontSize="12">
      ...
      />
      <SvgText x="200" y={y + 33} fill={palette.muted} fontSize="10">
        {item.meta ?? ""}
      </SvgText>
    </>
  );

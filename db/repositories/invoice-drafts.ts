...
import type { InvoiceDraft, InvoiceDraftInput, InvoiceDraftLine, InvoiceDraftListItem } from '@/types/invoice-draft';

type IdRow = { id: string };
type BusinessRow = {
  business_name: string; gstin: string | null; state_code: string | null; address: string;
  phone: string; email: string | null; logo_uri: string | null; signature_uri: string | null;
  payment_qr_uri: string | null; currency_code: string;
};
type CustomerRow = {
  id: string; name: string; phone: string | null; email: string | null; gstin: string | null;
  state_code: string | null; billing_address: string | null;
};
type DraftRow = {
  id: string; kind: 'tax_invoice' | 'non_tax_invoice'; customer_id: string | null;
  customer_name_snapshot: string | null; customer_state_code_snapshot: string | null;
  invoice_date: string; due_date: string | null; notes: string | null;
  business_state_code_snapshot: string | null;
};
type DraftLineRow = {
  id: string; item_id: string | null; item_type: 'product' | 'service';
  description_snapshot: string; sku_snapshot: string | null; unit_name_snapshot: string | null;
  quantity_scaled: number; unit_price_paise: number; cost_price_paise: number;
  discount_paise: number; gst_rate_basis_points: number;
};
type DraftListRow = {
  id: string; invoice_number: string; status: import('@/types/invoice-finalization').InvoiceRecordStatus;
  customer_name_snapshot: string | null; invoice_date: string;
  total_paise: number; updated_at: string;
};
...
export async function listInvoiceDrafts(): Promise<InvoiceDraftListItem[]> {
  ...
  return rows.map((row) => ({
    id: row.id, invoiceNumber: row.invoice_number, status: row.status,
    customerName: row.customer_name_snapshot, invoiceDate: row.invoice_date,
    totalPaise: row.total_paise, updatedAt: row.updated_at,
  }));
}
...
export async function loadInvoiceDraft(id: string): Promise<InvoiceDraft | null> {
  ...
  const draft = await database.getFirstAsync<DraftRow>(
    `SELECT id, kind, customer_id, customer_name_snapshot, customer_state_code_snapshot,
     ...
     FROM invoices WHERE id = ? AND status = 'draft'`, id,
  );
  ...
  const rows = await database.getAllAsync<DraftLineRow>(
    `SELECT id, item_id, item_type, description_snapshot, sku_snapshot, unit_name_snapshot,
     ...
     FROM invoice_items WHERE invoice_id = ? ORDER BY sort_order ASC`, id,
  );
  const lines: InvoiceDraftLine[] = rows.map((row) => ({
    id: row.id, itemId: row.item_id, itemType: row.item_type,
    description: row.description_snapshot, sku: row.sku_snapshot, unitName: row.unit_name_snapshot,
    quantityScaled: row.quantity_scaled, unitPricePaise: row.unit_price_paise,
    costPricePaise: row.cost_price_paise, discountPaise: row.discount_paise,
    gstRateBasisPoints: row.gst_rate_basis_points,
  }));
  return {
    id: draft.id, kind: draft.kind, customerId: draft.customer_id,
    customerName: draft.customer_name_snapshot, customerStateCode: draft.customer_state_code_snapshot,
    invoiceDate: draft.invoice_date, dueDate: draft.due_date, notes: draft.notes,
    businessStateCode: draft.business_state_code_snapshot, lines,
  };
}
  ...
  return runInTransaction(database, async (transaction) => {
    ...
    const customer = input.customerId
      ? await transaction.getFirstAsync<CustomerRow>(
          `SELECT id, name, phone, email, gstin, state_code, billing_address
           FROM customers WHERE id = ? AND is_archived = 0`, input.customerId,
        )
      : null;
    ...
    const calculation = calculateInvoice({
      ...
      lines: input.lines.map((line) => ({
        lineKey: line.id, quantityScaled: line.quantityScaled,
        unitPricePaise: line.unitPricePaise, discountPaise: line.discountPaise,
    ...
    const existing = input.id
      ? await transaction.getFirstAsync<IdRow>(
          `SELECT id FROM invoices WHERE id = ? AND status = 'draft'`, input.id,
        )
      : null;
    if (input.id && !existing) throw new Error('Draft is unavailable.');

    const values = [
      input.kind, customer?.id ?? null, input.invoiceDate, input.dueDate,
      customer?.name ?? null, customer?.phone ?? null, customer?.email ?? null,
      customer?.gstin ?? null, customer?.state_code ?? null, customer?.billing_address ?? null,
      business.business_name, business.gstin, business.state_code, business.address,
      business.phone, business.email, business.logo_uri, business.signature_uri,
      business.payment_qr_uri, business.currency_code, calculation.subtotalPaise,
      calculation.discountPaise, calculation.taxablePaise, calculation.cgstPaise,
      calculation.sgstPaise, calculation.igstPaise, calculation.roundingPaise,
      calculation.totalPaise, input.notes, timestamp,
    ] as const;
    ...
    if (existing) {
      ...
      await transaction.runAsync('DELETE FROM invoice_items WHERE invoice_id = ?', id);
    } else {
      await transaction.runAsync(
        ...
        id, `DRAFT-${id}`, ...values.slice(0, 29), timestamp, timestamp,
      );
    }
    ...
    for (let index = 0; index < input.lines.length; index += 1) {
      ...
      if (!line || !calculated) throw new Error('Invoice line calculation is unavailable.');
      ...
      await transaction.runAsync(
        ...
        persistedLineId, id, line.itemId, line.itemType, line.description, line.sku, line.unitName,
        line.quantityScaled, line.unitPricePaise, line.costPricePaise, line.discountPaise,
        line.gstRateBasisPoints, calculated.taxablePaise, calculated.cgstPaise,
        calculated.sgstPaise, calculated.igstPaise, calculated.lineTotalPaise, index,
      );
    }
    return id;
  });

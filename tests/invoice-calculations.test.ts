...
const intraState = calculateInvoice({
  kind: 'tax_invoice', businessStateCode: '09', customerStateCode: '09',
  roundToWholeRupee: false,
  lines: [{ lineKey: 'intra', quantityScaled: 1000, unitPricePaise: 10_000, discountPaise: 1000, gstRateBasisPoints: 1800 }],
});
...
const interState = calculateInvoice({
  kind: 'tax_invoice', businessStateCode: '09', customerStateCode: '27',
  roundToWholeRupee: false,
  lines: [{ lineKey: 'inter', quantityScaled: 1000, unitPricePaise: 10_000, discountPaise: 0, gstRateBasisPoints: 1800 }],
});
...
const quantityBoundary = calculateInvoice({
  kind: 'non_tax_invoice', businessStateCode: null, customerStateCode: null,
  roundToWholeRupee: false,
  lines: [{ lineKey: 'fractional', quantityScaled: 1500, unitPricePaise: 199, discountPaise: 0, gstRateBasisPoints: 1800 }],
});
assertEqual(quantityBoundary.subtotalPaise, 299, 'Half-up quantity multiplication');
assertEqual(quantityBoundary.taxPaise, 0, 'Non-tax invoice ignores GST');

const oddTax = calculateInvoice({
  kind: 'tax_invoice', businessStateCode: '09', customerStateCode: '09',
  roundToWholeRupee: false,
  lines: [{ lineKey: 'odd-tax', quantityScaled: 1000, unitPricePaise: 101, discountPaise: 0, gstRateBasisPoints: 500 }],
});
...
const roundDown = calculateInvoice({
  kind: 'non_tax_invoice', businessStateCode: null, customerStateCode: null,
  lines: [{ lineKey: 'down', quantityScaled: 1000, unitPricePaise: 101, discountPaise: 0, gstRateBasisPoints: 0 }],
});
...
const roundUp = calculateInvoice({
  kind: 'non_tax_invoice', businessStateCode: null, customerStateCode: null,
  lines: [{ lineKey: 'up', quantityScaled: 1000, unitPricePaise: 150, discountPaise: 0, gstRateBasisPoints: 0 }],
});
assertEqual(roundUp.roundingPaise, 50, 'Half-up rupee adjustment');
assertEqual(roundUp.totalPaise, 200, 'Half-up rupee total');

assertEqual(resolveInvoiceTaxMode('non_tax_invoice', null, null), 'none', 'Non-tax mode');
assertEqual(calculateOutstandingPaise(10_000, 2500), 7500, 'Outstanding amount');
assertThrows(() => resolveInvoiceTaxMode('tax_invoice', '09', null), 'Missing state');
assertThrows(() => calculateOutstandingPaise(100, 101), 'Overpayment');
assertThrows(() => calculateInvoice({
  kind: 'non_tax_invoice', businessStateCode: null, customerStateCode: null,
  lines: [{ lineKey: 'discount', quantityScaled: 1000, unitPricePaise: 100, discountPaise: 101, gstRateBasisPoints: 0 }],
}), 'Discount above subtotal');
assertThrows(() => calculateInvoice({
  kind: 'non_tax_invoice', businessStateCode: null, customerStateCode: null,
  lines: [
    { lineKey: 'duplicate', quantityScaled: 1000, unitPricePaise: 100, discountPaise: 0, gstRateBasisPoints: 0 },
    { lineKey: 'duplicate', quantityScaled: 1000, unitPricePaise: 100, discountPaise: 0, gstRateBasisPoints: 0 },
  ],
}), 'Duplicate line keys');
